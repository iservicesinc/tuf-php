...
class Router {
    ...
    public function init() {
        ...
        if ($route !== false) {
            $template_file = glob(BASE_DIR . '/views/' . $route['template'] . '*');
            if (substr($template_file[0], -5) === 'latte') {
                self::$latte->render($template_file[0], $route['params']);
            } else {
                include $template_file[0];
            }
        } else {
